...
# No additional config needed - data plane injected by modkit
...
pub enum ClientMode {
    SharedProcess { data_plane: Arc<dyn DataPlaneService> },
...
impl OagwClientConfig {
    pub fn from_env() -> Result<Self, ClientError> {
        match std::env::var("OAGW_MODE").as_deref() {
            Ok("shared") => {
                let data_plane = get_data_plane_from_di()?;
                Ok(Self {
...
impl SharedProcessClient {
    fn new(data_plane: Arc<dyn DataPlaneService>) -> Result<Self, ClientError> {
        Ok(Self { data_plane, metrics: Arc::new(Metrics::default()) })
    }
    ...
    async fn execute(&self, alias: &str, request: Request) -> Result<Response, ClientError> {
        ...
        let proxy_response = self.data_plane.proxy_request(proxy_request).await
    ...
    async fn websocket(&self, alias: &str, request: Request) -> Result<WebSocketConn, ClientError> {
        ...
        let ws_conn = self.data_plane.proxy_websocket(proxy_request).await
            .map_err(|e| ClientError::Connection(e.to_string()))?;
...
- Integration with `DataPlaneService` trait
    ...
    async fn test_deployment_agnostic_code() {
        let shared_config = OagwClientConfig {
            mode: ClientMode::SharedProcess {
                data_plane: get_mock_data_plane(),
            },
            default_timeout: Duration::from_secs(30),
        };
